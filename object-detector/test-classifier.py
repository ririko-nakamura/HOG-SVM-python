...
if __name__ == "__main__":
    ...
    parser.add_argument('-d', "--dataset", help="Path to the test dataset", required=True)
    parser.add_argument('-r','--ratio', help="Downscale ratio", default=1.25,
            type=int)
    ...
    downscale = args['ratio']
    visualize_det = args['visualize']

    # Load the classifier
    clf = joblib.load(model_path)

    for index in range(len(os.listdir(dataset))):

        if index % 100 != 0:
            continue

        image_fname = str(index+1) + ".jpg"
        ...
        for im_scaled in pyramid_gaussian(im, max_layer=0, downscale=downscale):
        ...
        #cv2.waitKey()
        filename, _ = os.path.splitext(image_fname)
        cv2.imwrite('./results/'+filename+"_result.jpg", clone)
